...
As described in the BioHackrXiv ELIXIR 2020 BioHackathon paper [@citesAsAuthority:extends:bhxiv20], metadata is crucial to publications, including acquiring a digital object identifier (DOI). DOIs are permanent URIs to PDFs, so publications can be cited by others. One interesting aspect is that DOIs support versioning - that means papers can be updated under the same DOI, this is not the case with content-addressable identifiers, such as the Interplanetary File System (IPFS).
